...
**End-to-end BI pipeline: Snowflake to Power BI**
...
|  |  |
|--|--|
| **Data Source** | Kaggle: US unemployment by state, monthly, 1976 to 2025 |
...
Snowflake (Azure West US 2)
  ...
  Schema:   LABOR_STATS
    ...
    - fact_unemployment   (31,747 rows: monthly, 1976 to 2025)
    |
    v
Power BI Desktop
...
Three KPI cards snapshot 2025 metrics with year-over-year movement. The trend line covers the full 1976 to 2025 range, making every major labor crisis visible. Right-click any year on the trend line to drill through to monthly detail.
...
**AREA_TYPE filter on all measures.** The dataset includes Los Angeles County and New York City as separate metro rows. Without filtering to `AREA_TYPE = "State"` they double-count on top of California and New York. The filter lives in the measure, not the visual.

**Yearly granularity on the trend line, not monthly.** 50 years × 12 months = 600 data points at page width is unreadable. Yearly (50 points) tells the trend cleanly. Monthly detail is available via drillthrough.

**Drillthrough instead of a slicer for monthly detail.** A slicer only filters the date range; it does not change chart granularity. Drillthrough is the correct Power BI pattern: summary on Page 1, monthly breakdown on Page 2 when the user drills into a specific year.
...
**Relationships:**
- `FACT_UNEMPLOYMENT[DATE]` to `Calendar[Date]`: many-to-one, active
- `FACT_UNEMPLOYMENT[FIPS_CODE]` to `DIM_AREA[FIPS_CODE]`: many-to-one, active
